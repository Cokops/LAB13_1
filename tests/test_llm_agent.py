...
def llm_agent():
    ...
    with patch('nats.connect') as mock_nats_connect, \
        ...
        agent.tracer_provider = MagicMock()
        agent.tracer_provider.shutdown = AsyncMock()
...
def test_handle_analyze_request_cache_hit(llm_agent):
    ...
    # Мокаем методы
    llm_agent._publish_result = MagicMock()
    llm_agent._generate_analysis = MagicMock()
    llm_agent._get_cached_result = AsyncMock(return_value={
        "cause": "Cached cause",
        "recommendation": "Cached fix",
        "criticality": "Medium"
    })
    llm_agent._publish_error_result = AsyncMock()
    ...
    llm_agent._publish_result.assert_called_once()
...
def test_handle_analyze_request_cache_miss(llm_agent):
    ...
    # Мокаем методы
    llm_agent._publish_result = MagicMock()
    llm_agent._get_cached_result = AsyncMock(return_value=None)
    llm_agent._generate_analysis = AsyncMock(return_value={
        "cause": "Generated cause",
        "recommendation": "Generated fix",
        "criticality": "High"
    })
    llm_agent._cache_result = AsyncMock()
    llm_agent._publish_error_result = AsyncMock()
    ...
    llm_agent._generate_analysis.assert_called_once()
    llm_agent._cache_result.assert_called_once()
    llm_agent._publish_result.assert_called_once()
...
def test_handle_analyze_request_no_anomaly(llm_agent):
    ...
    # Мокаем методы
    llm_agent._publish_error_result = AsyncMock()
    llm_agent._get_cached_result = AsyncMock()
    ...
    llm_agent._publish_error_result.assert_called_once_with(
        "No anomaly text provided", "req-999"
    )
...
@pytest.mark.asyncio
async def test_start(llm_agent):
    """Тест запуска агента"""
    # Проверяем что агент создан и имеет нужные атрибуты
    assert llm_agent.nc is not None
    assert llm_agent.redis_client is not None
    assert llm_agent.tracer_provider is not None

@pytest.mark.asyncio
async def test_close(llm_agent):
    """Тест закрытия соединений"""
    # Мокаем get_tracer_provider чтобы избежать ошибки
    with patch('llm_agent.llm_agent.trace.get_tracer_provider') as mock_get_tp:
        mock_tp = MagicMock()
        mock_tp.shutdown = AsyncMock()
        mock_get_tp.return_value = mock_tp
        
        await llm_agent.close()
        
        llm_agent.nc.close.assert_called_once()
